	...
	<table>
		<tr>
			<td>
				<!-- 처음 이전 링크 -->
				<c:if test="${ pg>1 }">
					[<a href="list.htm?pg=1">맨앞</a> ]
					[<a href="list.htm?pg=${ formPage }">이전</a> ]
				</c:if>
				
				<c:if test="${ pg<=1 }">
					[<span style="color: gray">맨앞</span> ]
				...
				<c:if test="${ pg < allPage }">
					[<a href="list.htm?pg=${ pg + 1 }">다음</a> ]
					[<a href="list.htm?pg=${ allPage }">맨뒤</a> ]
				</c:if>
				
				<c:if test="${ pg >= allPage }">
					[<span style="color: gray">다음</span> ]
					[<span style="color: gray;">맨뒤</span> ]
				</c:if>
